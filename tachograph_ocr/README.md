...
本リポジトリのソースコードの著作権は作者（内藤久美子 / Kumiko Naito）に帰属します。

本ソフトウェアはプロプライエタリであり、**商用利用・再配布・改変・派生物作成は厳禁**です。
利用には別途、作者からの書面による許諾が必要です。
...
- Core Implementation: Kumiko Naito
- Product Idea/Validation: Team D (SUNABACO PBL)

## 【デモ版について】

デモおよび評価用途として、以下を「正」とします。

- v0.1-demo (release/tag): `https://github.com/sapporoendo/tachograph_ocr/releases/tag/v0.1-demo`

`main` ブランチは開発中のため、動作保証はありません。デモ環境は `demo` ブランチの内容を基準として固定されます。
