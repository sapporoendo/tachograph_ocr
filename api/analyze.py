...
def extract_needle_mask(
    *,
    ...
    block_size = int(params.get("adaptiveBlockSize", 31))
    if block_size < 3:
        block_size = 3
    if (block_size % 2) == 0:
        block_size += 1
    C = int(params.get("adaptiveC", 2))
...
def estimate_segments_hsv_pencil_mask(
    ...
    *,
    ...
    roi_r = max(10, int(r * 0.92))
    ring_inner_ratio = 0.13
    ring_outer_ratio = 0.985
    h, w = bgr.shape[:2]
    x0 = max(cx - roi_r, 0)
    x1 = min(cx + roi_r, w)
    y0 = max(cy - roi_r, 0)
    ...
    inner_r = float(r) * float(ring_inner_ratio)
    outer_r = float(r) * float(ring_outer_ratio)
    outer_r = min(outer_r, float(roi_r))
    inner_r = min(inner_r, max(0.0, outer_r - 1.0))
    ...
    # Thin-stroke enhancement (pencil) using black-hat on grayscale.
    ...
    bh_vals = bh[ring_mask > 0]
    bh_p95 = float(np.percentile(bh_vals, 95.0)) if bh_vals.size > 0 else 0.0
    bh_thr = float(max(10.0, bh_p95))
    bh_mask = np.zeros_like(gray, dtype=np.uint8)
    bh_mask[bh >= bh_thr] = 255
    bh_mask[ring_mask == 0] = 0

    # HSV thresholds for low-saturation pencil. Start lenient to avoid nz==0 on white background.
    hsv_trials = [
        (20, 255, 60),
        (10, 255, 80),
        (0, 255, 120),
    ...
    for trial_idx, (v_min, v_max, s_max) in enumerate(hsv_trials):
        ...
        # Drop huge connected components (background / print fill).
        cc_min_area = 25
        ...
        if pencil_mask_nonzero == 0 and trial_idx == (len(hsv_trials) - 1):
            # If HSV pencil-like mask becomes empty, proceed without pencil mask gating.
        ...
        if xs.size == 0:
            ...
            cand = {
                ...
                "params": {
                    ...
                    "morphClose": int(k_close),
                    "roiR": int(roi_r),
                    "userMinCountPerDeg": 2.0,
                    "percentileP": 75.0,
                    "percentileValue": 0.0,
                    "minCountPerDeg": 0.0,
                    "effectiveMinCountPerDeg": 0.0,
                    "minCountPerDegMin": 1.0,
                    "minCountPerDegMax": 30.0,
                    "excludeDown": True,
                    "excludeDownCenterDeg": 270,
                    "excludeDownHalfWidthDeg": 20,
                    "gapDeg": 3,
                    "minSegDeg": 1,
                ...
                "diagnostics": {
                    ...
                    "blackhat": {"kernel": int(bh_ksize), "p95": float(bh_p95), "thr": float(bh_thr)},
        ...
        angle_bins = ang.astype(np.int32) % 360
        counts = np.bincount(angle_bins, minlength=360).astype(np.float32)
        ...
        zero_counts = np.bincount(angle_bins[in_zero], minlength=360).astype(np.float32)

        user_min_count_per_deg = 2.0
        percentile_p = 75.0
        use_p80 = False
        if use_p80:
            percentile_p = 80.0
        ...
        exclude_down = True
        exclude_down_center_deg = 270
        exclude_down_half_width_deg = 20
        if exclude_down:
            degs = np.arange(360, dtype=np.int32)
            diff = (degs - int(exclude_down_center_deg) + 540) % 360 - 180
            excluded = np.abs(diff) <= int(exclude_down_half_width_deg)
            counts[excluded] = 0.0
            zero_counts[excluded] = 0.0

        nonzero = counts[counts > 0.0]
        p50 = float(np.percentile(nonzero, 50.0)) if nonzero.size > 0 else 0.0
        p75 = float(np.percentile(nonzero, 75.0)) if nonzero.size > 0 else 0.0
        p90 = float(np.percentile(nonzero, 90.0)) if nonzero.size > 0 else 0.0

        perc = float(np.percentile(nonzero, float(percentile_p))) if nonzero.size > 0 else 0.0
        min_thr = float(max(1.0, float(user_min_count_per_deg)))
        max_thr = float(max(min_thr + 1.0, p90 * 1.20, perc * 0.90, 30.0))

        thr_base = float(max(p75 * 0.2, 5.0))
        thr = float(thr_base)
        active_deg_count_base = int(np.count_nonzero(counts >= thr))
        thr_iters = 0
        while active_deg_count_base > 220 and thr_iters < 10:
            thr *= 1.2
            thr_iters += 1
            active_deg_count_base = int(np.count_nonzero(counts >= thr))

        min_count_per_deg = float(min(max_thr, max(min_thr, float(thr))))
        active = counts >= min_count_per_deg
        ...
        if idxs.size == 0:
            trial_summaries.append(
                {
                    ...
                    "pencilMaskNonZero": int(pencil_mask_nonzero),
                    "usedNoPencilMask": bool(used_no_pencil_mask),
                    "minCountPerDegEffective": float(min_count_per_deg),
            ...
            cand = {
                ...
                "params": {
                    ...
                    "morphClose": int(k_close),
                    "roiR": int(roi_r),
                    "userMinCountPerDeg": float(user_min_count_per_deg),
                    "percentileP": float(percentile_p),
                    "percentileValue": float(perc),
                    "minCountPerDeg": float(min_count_per_deg),
                    "effectiveMinCountPerDeg": float(min_count_per_deg),
                    "minCountPerDegMin": float(min_thr),
                    "minCountPerDegMax": float(max_thr),
                    "excludeDown": bool(exclude_down),
                    "excludeDownCenterDeg": int(exclude_down_center_deg),
                    "excludeDownHalfWidthDeg": int(exclude_down_half_width_deg),
                    "gapDeg": int(gap_deg),
                    "minSegDeg": 1,
                ...
                "diagnostics": {
                    ...
                    "blackhat": {"kernel": int(bh_ksize), "p95": float(bh_p95), "thr": float(bh_thr)},
                    "ccFilter": {"minArea": int(cc_min_area), "maxArea": int(cc_max_area), "kept": int(kept_cc), "total": int(max(0, num_cc - 1))},
                    "countsPercentiles": {"p50": float(p50), "p75": float(p75), "p90": float(p90)},
                    "minCountPerDegEffective": float(min_count_per_deg),
                    "thrBase": float(thr_base),
                    "thrFinal": float(min_count_per_deg),
                    "thrIters": int(thr_iters),
                    "activeDegCountBase": int(active_deg_count_base),
                    "activeDegCount": int(active_deg_count),
                    "segmentsRawCount": 0,
                    "segmentsFinalCount": 0,
                },
        ...
        if runs and runs[0][0] <= gap_deg and runs[-1][1] >= 360 - gap_deg:
            merged = (runs[-1][0], runs[0][1])
            runs = [merged] + runs[1:-1]
        ...
        for a0, a1 in runs:
            seg_len_deg = a1 - a0
            if seg_len_deg < min_seg_deg:
                continue

            t0 = angle_to_time_24h(
                needle_angle_deg=float(a0),
                midnight_offset_deg=float(effective_midnight_offset),
            ...
            c = float(counts[a0:a1].sum()) if a1 > a0 else float(counts[a0:].sum() + counts[:a1].sum())
            zc = float(zero_counts[a0:a1].sum()) if a1 > a0 else float(zero_counts[a0:].sum() + zero_counts[:a1].sum())
            zero_ratio = float(zc / (c + 1e-6))
            ...
            segments.append(
                {
                    ...
                    "end": t1["timeHHMM"],
                    "type": seg_type,
                    "confidence": f"zeroRatio={zero_ratio:.2f}",
                    "durationMinutes": int(dur),
                    "startAngleDeg": int(a0),
                    "endAngleDeg": int(a1 % 360),
                    "twelveAngleOffsetDeg": float(twelve_angle_offset_deg),
        ...
        log = f"segments_hsv: runs={len(runs)} segs={len(segments)} v=[{v_min},{v_max}] sMax={s_max} minCountPerDeg={min_count_per_deg}"
        cand = {
            "segments": segments,
            "needsReviewMinutes": int(total_minutes),
            "params": {
